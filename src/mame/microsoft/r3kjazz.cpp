...
{
public:
	r3kjazz_state(const machine_config &mconfig, device_type type, const char *tag)
...
protected:
	...
	// machine config
	void r3kjazz(machine_config &config);

	void led_w(u8 data);

public:
	void r3000jazz(machine_config &config);
...
void r3kjazz_state::machine_start()
{
	m_led.resolve();
}

void r3kjazz_state::machine_reset()
{
	// HACK: make sure the RTC is running
	m_rtc->write_direct(0x0a, 0x20);
}

void r3kjazz_state::init_common()
{
...
void r3kjazz_state::cpu_map(address_map &map)
{
	map(0x00000000, 0xffffffff).rw(m_mct_adr, FUNC(mct_adr_device::r3k_r), FUNC(mct_adr_device::r3k_w));
}

void r3kjazz_state::mct_map(address_map &map)
{
	...
	map(0x8000f000, 0x8000f000).w(FUNC(r3kjazz_state::led_w));
...
void r3kjazz_state::r3kjazz(machine_config &config)
{
	...
	m_mct_adr->set_addrmap(0, &r3kjazz_state::mct_map);
	...
	NSCSI_CONNECTOR(config, "scsi:0", r3kjazz_scsi_devices, "harddisk");
	NSCSI_CONNECTOR(config, "scsi:1", r3kjazz_scsi_devices, nullptr);
	NSCSI_CONNECTOR(config, "scsi:2", r3kjazz_scsi_devices, nullptr);
...
void r3kjazz_state::led_w(u8 data)
{
...
void r3kjazz_state::r3000jazz(machine_config &config)
{
	r3kjazz(config);
...
ROM_START(r3000jazz)
	...
	//ROM_LOAD64_BYTE("r3kjazz_g364.bin", 0x000000, 0x020000, CRC(495fb417) SHA1(c341f3d498822ec1ee07a70076d7bbbf7aa60cb5) BAD_DUMP)
...
COMP(1991,  r3000jazz, 0,      0,      r3000jazz, 0,     r3kjazz_state, init_common, "Microsoft", "Jazz R3000", MACHINE_NO_SOUND)
